...
void elemallocator(struct elemdata ***p, int x, int y)
{
    int i;
    (*p) = (struct elemdata **)malloc(x * sizeof(struct elemdata *));
    for(i=0; i<x; i++)
    {
	(*p)[i] =  (struct elemdata *)malloc(y * sizeof(struct elemdata));
    }
}

void elemdeallocator(struct elemdata ***p, int x, int y)
{
    int i;
    for (i=0; i<x; i++)
    {
...
void dofallocator(struct dofdata **p, int x)
{
