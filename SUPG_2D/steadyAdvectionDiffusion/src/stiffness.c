...
void stiffness(double ***rhs, double ***phi, double **x, double **y)
{
    //------------------------------------------------------------------------//
    //Temporary Variables
    int ielem, jelem;
    ...
    //------------------------------------------------------------------------//
    //Array to store differential of weight functions
    double *wdiff1, *wdiff2;
    allocator1(&wdiff1, supgcoeff);
    allocator1(&wdiff2, supgcoeff);
    //------------------------------------------------------------------------//
	...
	for(jelem=2; jelem<yelem-2; jelem++)
	{
		...
		for(igauss=0; igauss<tgauss; igauss++)
		{
		    ...
		    //Get the weight differential vector - HAS TO BE CHANGED LATER
		    basisDiff2D(zeta[igauss][0], zeta[igauss][1],wdiff1, 1);
		    basisDiff2D(zeta[igauss][1], zeta[igauss][0],wdiff2, 2);
		    ...
		    term5 = pow(inv[0] + inv[1],2.0);
		    term6 = pow(inv[2] + inv[3],2.0);

		    //Add it all up
		    sum[icoeff] += weights[igauss][0]*weights[igauss][1]*mu*detJ*(term1+term2+term3+term4+term5+term6);
		}
	    }
    ...
    deallocator1(&wdiff1, supgcoeff);
    deallocator1(&wdiff2, supgcoeff);
